...
function listaTiposDeEventos() {
    $.ajax({
        url: servidor + 'tipoevento/consultar.php',
        dataType: 'json', 
        type: 'GET', 
...
function preencheTabela(dados) {
    ...
    table = $('#tabela').DataTable({
        ...
        columns: [
            ...
            { data: 'idtipoevento', visible: false },
    ...
    $('#tabela').on('click', '.btn-editar', function (e) {
        ...
        mostraFormulario(data.idtipoevento);
    });
    $('#tabela').on('click', '.btn-excluir', function (e) {
        var data = table.row($(this).parents('tr')).data();
        excluir(data.idtipoevento);
    });
}
...
function excluir(id){
    Swal.fire({
        ...
        text: "Deseja realmente excluir este tipo de evento?",
        icon: 'question',
        ...
        if (result.value) {
            ...
            excluirTipoDeEvento(id);
        }
    });
}

// Função que altera a senha do usuário logado
function excluirTipoDeEvento(id) {
    $.ajax({
     url: servidor + 'tipoevento/excluir.php', 
