...
txtSenha.addEventListener("keyup", pressionarEnter);
...
// Função que verifica se o usuário e a senha existem
function logar() {
    // Captura os dados digitados pelo usuário
    let login = document.querySelector(`#txtLogin`).value.toLowerCase();
    let senha = document.querySelector(`#txtSenha`).value;
    $.ajax({
        // Servidor onde os dados serão enviadas
        url: servidor + 'usuario/consultarporloginesenha.php',
        ...
        success: function (dados) {
            ...
            if (dados.resp == 1) { // E-mail e senha Ok
                // Cria uma sessão local no navegador
                sessionStorage.setItem("idLogado", dados.id);
...
var hex_chr = "dsFDlaDhDfjDash4z76vcxzTD65fsahdsafDUdHsJhauKfaJoigUYAnji";
function rhex(num) {
    str = "";
    for (j = 0; j <= 3; j++)
