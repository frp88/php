...
function listaTarefas() {
    $.ajax({
        url: servidor + 'tarefa/consultar.php',
        dataType: 'json',
        type: 'GET',
...
function preencheTabela(dados) {
    ...
    table = $('#tabela').DataTable({
        ...
        columns: [
            ...
            { data: 'idtarefa', visible: false },
            { data: 'tituloevento', title: "Evento" },
            {
    ...
    $('#tabela').on('click', '.btn-editar', function (e) {
        ...
        mostraFormulario(data.idtarefa);
    });
    $('#tabela').on('click', '.btn-excluir', function (e) {
        var data = table.row($(this).parents('tr')).data();
        excluir(data.idtarefa);
    });
}
...
function preencheTabelaOld(dados) {
    ...
    <th class="whitespace-no-wrap">EVENTO</th>
    ...
    if (dados != null) {
        for (let item in dados) {
            ...
            lista.innerHTML += (`<tr>
                ...
                <td>${dados[item].tituloevento != null ? dados[item].tituloevento : ''}</td>
                ...
                <a class="flex items-center mr-3" title="Alterar" href="#" onclick="mostraFormulario(${dados[item].idtarefa})"> <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="feather feather-check-square w-4 h-4 mr-1"><polyline points="9 11 12 14 22 4"></polyline><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"></path></svg>
                 Alterar </a>
                 <a class="flex items-center text-theme-6" title="Excluir"  href="#" onclick="excluir(${dados[item].idtarefa})" data-toggle="modal" data-target="#delete-confirmation-modal"> 
        ...
        lista.innerHTML += `<tr><td colspan="6" class="text-center">Nenhuma tarefa cadastrada...</td></tr>`;
    }
...
function excluir(id) {
    Swal.fire({
        title: 'Excluir tafera?',
        text: "Deseja realmente excluir este tarefa?",
        icon: 'question',
...
function excluirTarefa(id) {
    $.ajax({
        url: servidor + 'tarefa/excluir.php',
