...
function listaTiposDeServicos() {
    $.ajax({
        url: servidor + 'tiposervico/consultar.php',
        dataType: 'json',
        type: 'GET',
...
function preencheTabela(dados) {
    ...
    table = $('#tabela').DataTable({
        ...
        columns: [
            ...
            { data: 'idtiposervico', visible: false },
        ...
        buttons: [
            {
                ...
                title: "Lista de Tipos de Serviços",
    ...
    $('#tabela').on('click', '.btn-editar', function (e) {
        ...
        mostraFormulario(data.idtiposervico);
    });
    $('#tabela').on('click', '.btn-excluir', function (e) {
        var data = table.row($(this).parents('tr')).data();
        excluir(data.idtiposervico);
    });
}
...
function excluir(id) {
    Swal.fire({
        ...
        text: "Deseja realmente excluir este tipo de serviço?",
        icon: 'question',
        ...
        if (result.value) {
            ...
            excluirTipoDeServico(id);
        }
    });
}

// Função que altera a senha do usuário logado
function excluirTipoDeServico(id) {
    $.ajax({
        url: servidor + 'tiposervico/excluir.php',
